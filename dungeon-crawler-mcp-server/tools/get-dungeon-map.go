...
func generateASCIIMap(player *types.Player, dungeon *types.Dungeon) string {
	...
	// Create a grid to track room information
	...
	for displayY := 0; displayY < dungeon.Height; displayY++ {
		realY := dungeon.Height - 1 - displayY  // Reconvertir pour l'affichage
		...
		for x := 0; x < dungeon.Width; x++ {
			...
			if roomInfo.Visited {
				builder.WriteString("       ")  // Ligne vide pour les pièces visitées
			} else {
				builder.WriteString(" ???   ")  // 7 caractères pour alignement
			}
			if x < dungeon.Width-1 {
				builder.WriteString("│")
			}
